...
def focal_heatmap_loss(a, h, gamma=2.0, epsilon=1e-8):
    ...
    assert (h >= 0).all(), (
        f"heatmap should be in [0, 1], but got min value: {h.min().item()}"
    )
    assert (h <= 1).all(), (
        f"heatmap should be in [0, 1], but got max value: {h.max().item()}"
    )
...
def gaussian_heatmap(
    ...
    y_grid, x_grid = torch.meshgrid(y_coords, x_coords, indexing="ij")
    y_grid, x_grid = (
        repeat(a, "h w -> n s h w", n=1, s=1).contiguous() for a in (y_grid, x_grid)
    )

    center_x, center_y = (a * target_width / width for a in (center_x, center_y))
    center_x, center_y = (
        repeat(a, "n -> n s h w", s=1, h=1, w=1).contiguous().to(device)
        for a in (center_x, center_y)
    )
    sigma = repeat(sigma, "s -> n s h w", n=1, h=1, w=1).to(device).contiguous()
...
class HeatmapFocalLoss(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert len(sigmas) == len(stages), (
            f"sigmas should have the same length with stages, but got {len(sigmas)} and {len(stages)}"
        )

        # cfg = Config.fromfile(dw_pose_cfg)
        init_default_scope("mmpose")
        self.model: TopdownPoseEstimator = init_model(
            dw_pose_cfg, dw_pose_ckpt, device="cpu"
        )
        ...
        if stage_lambda is not None:
            self.stage_lambda = nn.Parameter(
                torch.tensor(stage_lambda), requires_grad=False
            )
        else:
            self.stage_lambda = nn.Parameter(
                torch.ones(len(stages), dtype=torch.float32), requires_grad=False
            )
    ...
    HeatmapFocalLossOut = namedtuple(
        "HeatmapFocalLossOut", ["out", "ctc_loss", "heatmap_focal_loss"]
    )
    ...
    def forward(self, outputs, input, input_length, target, target_length):
        ...
        if self.ctc_weight > 0.0:
            ...
            ctc_loss = self.ctc_weight * ctc_loss

        if self.heatmap_weight > 0.0:
            heatmap_loss, _ = self._loss_heatmap(
                outputs.encoder_out.attn_weights, input
            )
            heatmap_loss = self.heatmap_weight * heatmap_loss

        return self.HeatmapFocalLossOut(
            out=heatmap_loss + ctc_loss,
            ctc_loss=ctc_loss,
            heatmap_focal_loss=heatmap_loss,
        )

    def _loss_heatmap(self, attention_maps, input):
        """
        @param attention maps: [t n s head keys h w] s means Stage!
        @param input: [n c t h w]
        @retrun loss: scalar, target_heatmap: [n s h w]
        """
        attention_maps = attention_maps[:, :, self.stages]
        attention_maps = rearrange(
            attention_maps, "t n s heads keys h w -> (n t) s heads keys h w"
        )
...
if __name__ == "__main__":
    ...
    attention = rearrange(
        attention, "t n s heads keys (h w) -> t n s heads keys h w", h=8, w=8
    )
